...
# ==========================================
# 3. MEMORY CACHE SYSTEM (Race Condition ဖြေရှင်းထားသည်)
# ==========================================
CACHE_PRODUCTS = {}
CACHE_ACCOUNTS = {}
CACHE_PENDING = {}
...
def init_data():
    global CACHE_PRODUCTS, CACHE_ACCOUNTS, CACHE_PENDING
    
    # Load Products
    if os.path.exists(PRODUCTS_FILE):
        try:
            with open(PRODUCTS_FILE, "r", encoding="utf-8") as f:
                CACHE_PRODUCTS = json.load(f)
        except:
            CACHE_PRODUCTS = copy.deepcopy(default_products)
    else:
        CACHE_PRODUCTS = copy.deepcopy(default_products)
        
    changed = False
    for k, v in default_products.items():
        if k not in CACHE_PRODUCTS:
            CACHE_PRODUCTS[k] = copy.deepcopy(v)
            changed = True
        else:
            if CACHE_PRODUCTS[k]['name'] != v['name']:
                CACHE_PRODUCTS[k]['name'] = v['name']
                changed = True
    if changed:
        save_cache(PRODUCTS_FILE, CACHE_PRODUCTS)

    # Load Accounts
    if os.path.exists(ACCOUNTS_FILE):
        try:
            with open(ACCOUNTS_FILE, "r", encoding="utf-8") as f: CACHE_ACCOUNTS = json.load(f)
        except: pass
        
    # Load Pending
    if os.path.exists(PENDING_FILE):
        try:
            with open(PENDING_FILE, "r", encoding="utf-8") as f: CACHE_PENDING = json.load(f)
        except: pass
...
def log_sale(user_id, product_name, qty, total):
    ...
    with file_lock:
        ...
        if os.path.exists(SALES_FILE):
            try:
                with open(SALES_FILE, "r") as f: admin_data = json.load(f)
            except: pass
        if date_str not in admin_data: admin_data[date_str] = []
        admin_data[date_str].append(sale_entry)
        
        tmp_path = SALES_FILE + ".tmp"
        with open(tmp_path, "w", encoding="utf-8") as f: json.dump(admin_data, f, indent=4)
        os.replace(tmp_path, SALES_FILE)
        
        user_history = {}
        if os.path.exists(HISTORY_FILE):
            try:
                with open(HISTORY_FILE, "r") as f: user_history = json.load(f)
            except: pass
        str_uid = str(user_id)
        if str_uid not in user_history: user_history[str_uid] = []
        user_history[str_uid].append(sale_entry)
        
        tmp_path_h = HISTORY_FILE + ".tmp"
        with open(tmp_path_h, "w", encoding="utf-8") as f: json.dump(user_history, f, indent=4)
        os.replace(tmp_path_h, HISTORY_FILE)

def get_and_deduct_accounts(product_key, qty, user_id):
    if product_key not in CACHE_ACCOUNTS: return None
    assigned = []
    needed = qty
    for acc in CACHE_ACCOUNTS[product_key]:
        if needed == 0: break
    ...
    CACHE_ACCOUNTS[product_key] = [acc for acc in CACHE_ACCOUNTS[product_key] if acc['uses_left'] > 0]
    save_cache(ACCOUNTS_FILE, CACHE_ACCOUNTS)
...
def generate_hiddify_links(days, gb, user_id):
    ...
    try:
        ...
        if response.status_code in [200, 201]: 
            link1 = f"{client_base_url}{new_uuid}/#{profile_name}"
            link2 = f"{client_base_url}{new_uuid}/sub/#{profile_name}"
            return link1, link2
...
def process_approval(order_id, data):
    ...
    key = data['key']
    qty_sold = data['qty']
    p_info = CACHE_PRODUCTS.get(key)
    
    if not p_info: return
    
    # Stock ဖြတ်ခြင်း (Memory မှ တိုက်ရိုက်လုပ်ဆောင်သည်)
    if p_info['stock'] > 0: 
...
def admin_command(m):
    if m.from_user.id == ADMIN_ID:
        admin_panel_view(m.chat.id)

def get_admin_product_menu(action):
    markup = types.InlineKeyboardMarkup(row_width=1)
    for k, p in CACHE_PRODUCTS.items():
...
def step_setstock(m, key):
    try:
        new_stock = int(m.text)
        CACHE_PRODUCTS[key]['stock'] = new_stock
        save_cache(PRODUCTS_FILE, CACHE_PRODUCTS)
        bot.send_message(m.chat.id, f"✅ <b>{CACHE_PRODUCTS[key]['name']}</b> အတွက် Stock အသစ် <b>{new_stock}</b> သို့ ပြောင်းလဲပြီးပါပြီ။", parse_mode="HTML")
    except:
...
def step_edit_price(m, key):
    try:
        ...
        CACHE_PRODUCTS[key]['price'] = new_price
        save_cache(PRODUCTS_FILE, CACHE_PRODUCTS)
        bot.send_message(m.chat.id, f"✅ <b>{CACHE_PRODUCTS[key]['name']}</b> အတွက် ဈေးနှုန်းအသစ် <b>{new_price} Ks</b> သို့ ပြောင်းလဲသတ်မှတ်ပြီးပါပြီ။", parse_mode="HTML")
    except:
...
def step_addacc_uses(m):
    try:
        ...
        if key not in CACHE_ACCOUNTS: CACHE_ACCOUNTS[key] = []
        
        for a in acc_list:
            if a.strip():
                CACHE_ACCOUNTS[key].append({"data": a.strip(), "uses_left": uses, "used_by": []})
        save_cache(ACCOUNTS_FILE, CACHE_ACCOUNTS)
        
        added_stock = len(acc_list) * uses
        if CACHE_PRODUCTS[key]['stock'] == -1: CACHE_PRODUCTS[key]['stock'] = 0
        CACHE_PRODUCTS[key]['stock'] += added_stock
        save_cache(PRODUCTS_FILE, CACHE_PRODUCTS)
...
def get_menu(cat="main", page=0, user_id=None):
    ...
    if cat == "main":
        ...
        keys = [k for k in CACHE_PRODUCTS.keys() if k.startswith(pre)]
...
def handle_photo(message):
    ...
    if uid in user_orders and "awaiting_slip" in user_orders[uid]:
        ...
        if oid in CACHE_PENDING:
            ...
            caption = f"🔔 <b>ငွေလွှဲပြေစာ အသစ်!</b>\n\nOrder ID: <code>{oid}</code>\nUser: {uid}\nကျသင့်ငွေ: {CACHE_PENDING[oid]['total']} Ks\nProduct: {CACHE_PENDING[oid]['key']}"
...
def cb(call):
    ...
    try:
        ...
        if call.data == "open_admin_panel":
            ...
            accs = CACHE_ACCOUNTS.get(key, [])
            if not accs:
                bot.send_message(cid, "ဤပစ္စည်းအတွက် သိမ်းဆည်းထားသော အကောင့်မရှိပါ။")
            else:
                text = f"📋 <b>အကောင့်စာရင်း ({len(accs)} ခု)</b>\n\n"
                for i, a in enumerate(accs, 1):
                    text += f"{i}. <code>{a['data']}</code> (ကျန်: {a['uses_left']} ခါ)\n"
        ...
        # --- USER CALLBACKS ---
            ...
            if cat == "cat_evpn" or cat.startswith("cat_ev_"):
                menu_text = "🛡️ <b>Express VPN</b>\n\nလိုအပ်သော Plan ကို ရွေးချယ်ပါ -"
            elif cat == "cat_hiddify":
                menu_text = "🚀 <b>Hiddify VPN</b>\n\nလိုအပ်သော Plan ကို ရွေးချယ်ပါ -"
            elif cat == "cat_capcut":
                menu_text = "✂️ <b>Capcut Premium</b>\n\nလိုအပ်သော Plan ကို ရွေးချယ်ပါ -"
            ...
            markup = types.InlineKeyboardMarkup(row_width=1)
            for i in range(1, 4): 
                markup.add(types.InlineKeyboardButton(f"အရေအတွက်: {i} ခု", callback_data=f"qty_{i}"))
            ...
            markup = types.InlineKeyboardMarkup(row_width=1)
            markup.add(
                types.InlineKeyboardButton("Wave Pay ဖြင့်ချေမည်", callback_data="p_w"), 
                types.InlineKeyboardButton("KBZ Pay ဖြင့်ချေမည်", callback_data="p_k"),
                types.InlineKeyboardButton("⬅️ ပင်မမီနူးသို့", callback_data="page_main_0")
            )
            ...
            CACHE_PENDING[order['oid']] = {"user_id": uid, "key": order['key'], "qty": order['qty'], "total": total}
            save_cache(PENDING_FILE, CACHE_PENDING)
            
            user_orders[uid]["awaiting_slip"] = order['oid']
            
            warranty_msg = "🛡️ <b>Full Warranty</b> ပေးထားပါသဖြင့် ယုံကြည်စိတ်ချစွာ ဝယ်ယူနိုင်ပါသည်။"
            if order['key'] == "canva":
                warranty_msg = "🛡️ <b>Warranty 1 Year</b> ပေးထားပါသဖြင့် ယုံကြည်စိတ်ချစွာ ဝယ်ယူနိုင်ပါသည်။"
            ...
            msg = "📋 <b>သင်၏ ဝယ်ယူမှုမှတ်တမ်းများ</b>\n\n" + "\n".join([f"📦 {h['product']} - {h['total']} Ks" for h in user_hist[-5:]]) if user_hist else "မှတ်တမ်း မရှိသေးပါ။"
            markup = types.InlineKeyboardMarkup(row_width=1)
            markup.add(types.InlineKeyboardButton("🏠 ပင်မမီနူးသို့", callback_data="page_main_0"))
