...
default_products = {
    ...
    # Hiddify အတွက် သတ်မှတ်ထားသော Stock ကိုသုံးမည်ဖြစ်၍ default 0 ထားသည် (Admin မှ stock ပြင်ပေးရမည်)
...
# --- ပို့ပေးမည့် အကောင့်များကို လိုအပ်သလို Format ချပေးမည့် Function ---
def format_acc_data(key, raw_data):
    # | သို့မဟုတ် : ဖြင့် ခြားထားသော data များကို ခွဲထုတ်မည်
    ...
    if "gpt" in key:
        ...
        return f"Key - <code>{raw_data.strip()}</code>"
        
    else: # အခြားအရာများ (Capcut, Mobile VPN, etc)
        if len(parts) >= 2:
...
def cb(call):
    ...
    try:
        ...
        # --- USER CALLBACKS ---
            ...
            if user_orders[uid]["key"] == "canva":
                msg = bot.send_message(cid, "📧 သင့်၏ Canva အသုံးပြုမည့် Email ကို စာရိုက်၍ ပပို့ပါ:")
