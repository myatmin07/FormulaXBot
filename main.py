...
def process_approval(order_id, data):
    ...
    if len(delivered) == qty_sold:
        ...
        try: bot.send_message(target_id, f"🎉 <b>Your order has been approved!</b>{acc_text}", parse_mode="HTML")
        except: pass
        try: bot.send_message(ADMIN_ID, f"✅ Successfully delivered Order <code>{order_id}</code> to User {target_id}.")
        except: pass
    else:
        try: bot.send_message(target_id, f"🎉 <b>Your order has been approved!</b>\n\n⚠️ <b>Please wait a moment.</b>\n\nAccounts are currently being restocked. The admin will manually send your account details shortly. Please kindly contact @FORMULA_X0.", parse_mode="HTML")
        except: pass
        try: bot.send_message(ADMIN_ID, f"⚠️ User: {target_id}\n<b>Out of Stock / API Error. Please manually send the details.</b>\nProduct: {p_info['name']}")
        except: pass

# ==========================================
# 5. AUTO APPROVAL WEBHOOK (Flask API)
# ==========================================
app = Flask(__name__)
...
def payment_webhook():
    data = request.json
    if not data or data.get("secret") != "formula_x_auto_123":
        return jsonify({"error": "Unauthorized"}), 403
    
    noti_text = data.get("text", "")
    match = re.search(r'FX-[A-Z0-9]{5}', noti_text)
    ...
    if match:
        ...
        if order_id in pending:
            ...
            bot.send_message(ADMIN_ID, f"⚡ <b>Auto Approval Triggered!</b>\n\nငွေဝင် Notification လက်ခံရရှိပါသည်။\nOrder ID: <code>{order_id}</code>\nProduct: {order_data['key']}\n\nစနစ်မှ အလိုအလျောက် Account ပေးပို့နေပါပြီ...", parse_mode="HTML")
            process_approval(order_id, order_data)
            del pending[order_id]
            save_pending(pending)
            return jsonify({"status": "success", "message": f"Auto approved {order_id}"}), 200
        else:
            return jsonify({"status": "ignored", "message": "Order not found in pending list"}), 200
    else:
        return jsonify({"status": "ignored", "message": "No FX-XXXXX format found in text"}), 200
...
def step_setstock(m, key):
    try:
        ...
        with db_lock: PRODUCTS_DB[key]['stock'] = new_stock
        save_products_db()
...
def step_edit_price(m, key):
    try:
        ...
        with db_lock: PRODUCTS_DB[key]['price'] = new_price
        save_products_db()
...
def handle_photo(message):
    ...
    if uid in user_orders and "awaiting_slip" in user_orders[uid]:
        ...
        if oid in pending:
            ...
            bot.reply_to(message, "✅ <b>Payment slip received successfully.</b>\n\nWe will process it immediately (Auto Approval is running).", parse_mode="HTML")
...
def cb(call):
    ...
    try:
        if call.data == "open_admin_panel":
        ...
        elif call.data == "admin_editprice": bot.edit_message_text("💰 Select the product to edit its price:", cid, mid, reply_markup=get_admin_product_menu("price"))
        elif call.data == "admin_setstock": bot.edit_message_text("📦 Select the product to manage its stock:", cid, mid, reply_markup=get_admin_product_menu("stk"))
        elif call.data == "admin_addacc": bot.edit_message_text("➕ Select the product to add new accounts:", cid, mid, reply_markup=get_admin_product_menu("addacc"))
        elif call.data == "admin_viewacc": bot.edit_message_text("👀 Select the product to view saved accounts:", cid, mid, reply_markup=get_admin_product_menu("vw"))
            ...
            p = PRODUCTS_DB[key]
            if "cgpt" in key or "ai_" in key or "grok" in key: prompt_text = f"Please send <b>Email and Passwords</b> for <b>{p['name']}</b>.\n\n(Send one per line using the format: <b>Email|Email_Pass|GPT_Pass</b>)\nExample:\nuser@outlook.com|pass123|gpt456"
            elif key.startswith("evpn_pc_"): prompt_text = f"Please send <b>License Keys</b> for <b>{p['name']}</b>.\n\n(Send one per line)\nExample:\nEXYZ123456789"
            elif p.get('is_hiddify') or "hid_" in key: prompt_text = f"Please send <b>VPN Links</b> for <b>{p['name']}</b>.\n\nExample:\nvless://..."
            else: prompt_text = f"Please send <b>Email and Passwords</b> for <b>{p['name']}</b>.\n\n(Send one per line using the format: <b>Email|Password</b>)\nExample:\nuser@gmail.com|pass123"
        ...
        elif call.data == "oos": bot.answer_callback_query(call.id, "Sorry, this item is currently out of stock.", show_alert=True)
...
if __name__ == "__main__":
    ...
    print("🤖 FormulaX Bot is successfully running with Auto-Approval API on port 5000...")
    while True:
        try:
