...
# SSL Warning များကို ဖျောက်ထားရန် (Hiddify API အတွက်)
...
default_products = {
    ...
    "hid_hk_1m": {"name": "🇭🇰 Hiddify VPN 1 M (100GB)", "price": 3000, "stock": -1, "prefix": "FX-HK1", "is_hiddify": True, "days": 30, "gb": 100},
    "hid_hk_2m": {"name": "🇭🇰 Hiddify VPN 2 M (200GB)", "price": 6000, "stock": -1, "prefix": "FX-HK2", "is_hiddify": True, "days": 60, "gb": 200},
    "hid_hk_3m": {"name": "🇭🇰 Hiddify VPN 3 M (300GB)", "price": 9000, "stock": -1, "prefix": "FX-HK3", "is_hiddify": True, "days": 90, "gb": 300},
}
    ...
    for k, v in default_products.items():
        if k not in data:
            ...
            if data[k]['name'] != v['name']:
                data[k]['name'] = v['name']
                changed = True
            # Hiddify များ Error မတက်စေရန် Stock အား -1 (အကန့်အသတ်မရှိ) သို့ ပြောင်းပေးမည်
            if v.get('is_hiddify') and data[k]['stock'] == 0:
                data[k]['stock'] = -1
                changed = True
    if changed: save_products(data)
    return data
...
def generate_hiddify_links(days, gb, user_id):
    ...
    try:
        ...
        if response.status_code in [200, 201]: 
            ...
            # လင့်ခ် ၂ အား Base64 Standard Sub လင့်ခ်အဖြစ် ပြောင်းလဲထားသည်
...
def process_approval(order_id, data):
    ...
    if p_info.get('is_hiddify'):
        for _ in range(qty_sold):
            ...
            if link1 and link2: 
                ...
                if accs: delivered.append(f"🌐 <b>Server Link:</b>\n<code>{accs[0]}</code>")
    else:
    ...
    if len(delivered) == qty_sold:
        if p_info.get('is_hiddify'):
            ...
            acc_text = "\n\n🔑 <b>အကောင့်အချက်အလက်များ:</b>\n\n" + "\n\n".join([f"{i}. <code>{a}</code>" for i, a in enumerate(delivered, 1)])
        ...
        bot.send_message(target_id, f"🎉 <b>အော်ဒါ အတည်ပြုပြီးပါပြီ။</b>\n\n⚠️ သို့သော် စနစ်အတွင်း Stock မလုံလောက်သဖြင့် Admin မှ အကောင့်ကို ခေတ္တနေလျှင် ကိုယ်တိုင် ပေးပို့ပေးပါမည်။", parse_mode="HTML")
...
def step_addacc_uses(m):
    try:
        ...
        for a in acc_list:
            if a.strip():
                accs_db[key].append({"data": a.strip(), "uses_left": uses, "used_by": []})
...
def confirm(cid, uid):
    ...
    # Warranty စာသား ထည့်သွင်းခြင်း
    warranty_text = "🛡️ <b>Full Warranty</b>"
    if order['key'] == "canva":
        warranty_text = "🛡️ <b>Warranty 1 Year</b>"
...
def get_menu(cat="main", page=0, user_id=None):
    ...
    if cat == "main":
        ...
        for k in page_keys:
            p = prods[k]
            s = f" [{p['stock']}]" if p['stock'] != -1 and not p.get('is_hiddify') else ""
            markup.add(types.InlineKeyboardButton(f"{p['name']} • {p['price']} Ks{s}", callback_data=f"buy_{k}" if p['stock'] != 0 else "oos"))
...
def cb(call):
    ...
    try:
        ...
        if call.data == "open_admin_panel":
            ...
            # ပစ္စည်းအမျိုးအစားအလိုက် တောင်းဆိုမည့် စာသားခွဲခြားခြင်း
            if p.get('is_hiddify') or "hid_" in key:
                prompt_text = f"<b>{p['name']}</b> အတွက် <b>VPN Link များ</b> ပေးပို့ပါ။\n\n(တစ်ခုကို တစ်ကြောင်းစီ ခွဲ၍ ပို့နိုင်ပါသည်။ ဥပမာ:\nvless://...\nvmess://...)"
        ...
        # --- USER CALLBACKS ---
            ...
            # စာသားပြောင်းလဲပေးခြင်းဖြင့် Telegram ၏ "Message not modified" error ကို တားဆီးသည်
            ...
            # Payment Message တွင် Warranty စာသားများ ထည့်သွင်းခြင်း
        ...
        # Message Not Modified Error အား တားဆီးထားသည်
