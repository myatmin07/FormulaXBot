...
def pop_stock(prod, qty=1):
    ...
    accs = [db[prod].pop(0) for _ in range(qty)]
...
def get_product_buttons(prod_list, back_target):
    ...
    for prod_id, duration, price, title in prod_list:
        ...
        if stock:
            if prod_id in ["spotify_2", "spotify_3"]:
                total_days = 60 if prod_id == "spotify_2" else 90
                acc = stock[0]
                ts = acc.get("timestamp", time.time())
                days_passed = int((time.time() - ts) / 86400)
                days_left = max(0, total_days - days_passed)
                btn_text = f"Buy {title} | {price:,} MMK | {tg_emoji(E_DAYS_ID, '⏳')} {days_left} Days Left"
            else:
...
@bot.callback_query_handler(func=lambda call: call.data.startswith(('approve_', 'reject_', 'apw_', 'rjw_')))
def admin_action_handler(call):
    uid, cid, mid = call.from_user.id, call.message.chat.id, call.message.message_id
    if uid != ADMIN_ID:
        try: bot.answer_callback_query(call.id, "Admin Access Only.", show_alert=True)
        except: pass
        return
    ...
    if data.startswith("reject_") or data.startswith("rjw_"):
    ...
    if data.startswith("apw_"):
        parts = data.split("_")
        user_id = parts[1]
        prod_type = parts[2]
        gb_val = int(parts[3])
        total_price = int(parts[4]) if len(parts) > 4 else 0
        ...
        if prod_type == "outline":
            ...
            if links:
                ...
                add_usage(gb_val)
                add_history(user_id, {"oid": "WEB", "product": "Outline VPN", "qty": 1, "total": total_price, "date": date_str})
            else:
                bot.send_message(ADMIN_ID, f"{E_WARN} Error: API failed for Web Order (Outline).")
            ...
            if links:
                ...
                add_usage(gb_val)
                add_history(user_id, {"oid": "WEB", "product": "Hiddify VPN", "qty": 1, "total": total_price, "date": date_str})
            else:
                bot.send_message(ADMIN_ID, f"{E_WARN} Error: API failed for Web Order (Hiddify).")
                bot.edit_message_caption(f"{caption}\n\n❌ <b>FAILED:</b> API Error.", cid, mid, parse_mode="HTML")
        ...
        if prod_type in ["out", "hid"]:
            ...
            if prod_type == "capcut":
                ...
                for i, acc in enumerate(accs):
                    ...
                    if email_match:
                        markup_keys.append([{"text": f"Mail Access {i+1}", "url": f"https://generator.email/{email_match.group(0)}", "icon_custom_emoji_id": E_EMAIL_ID}])
...
def start_cmd(m):
    if not is_authorized(m.from_user.id):
        bot.send_message(m.chat.id, f"{E_WARN} <b>Access Denied</b>\n\nYou need an access key to use this bot.\nPlease enter your access key or contact @FORMULA_X0 to purchase one.", parse_mode="HTML", reply_markup=ReplyKeyboardMarkup())
        return
    ...
    btn_store = KeyboardButton(text="🛍️ Digital Store", web_app=WebAppInfo(url=WEBAPP_URL))
    reply_markup.add(btn_store, KeyboardButton("🔔 Contact Admin"))
    reply_markup.add(KeyboardButton("👤 My Info"), KeyboardButton("📊 Order History"))
    btn_checker = KeyboardButton(text="🔍 Outline Key Check", web_app=WebAppInfo(url=f"{WEBAPP_URL}?view=checker"))
    reply_markup.add(btn_checker)
    ...
    bot.send_message(m.chat.id, f"{E_STORE} <b>Discover our products:</b>", parse_mode="HTML", reply_markup=get_main_menu(m.from_user.id))

@bot.message_handler(func=lambda m: m.text in ["🔔 Contact Admin", "👤 My Info", "📊 Order History"])
def text_reply(m):
    uid = m.from_user.id
    ...
    if m.text == "🔔 Contact Admin":
        ...
        if history:
            for r in history[-10:]:
                qty = r.get('qty', 1)
                text += f"• #{r.get('oid', 'WEB')} {r.get('product', '')} ({qty}x) - {r.get('total', 0):,} MMK\n"
        else:
...
def auth_guard(m):
    ...
    if key in keys_db:
        ...
        bot.send_message(m.chat.id, f"{E_VERIFIED} <b>Access Granted!</b>\nWelcome to Formula-X Hub. Send /start to begin.", parse_mode="HTML", reply_markup=reply_markup)
    else:
        bot.send_message(m.chat.id, f"{E_WARN} <b>Invalid Key</b>\nPlease contact @FORMULA_X0.", parse_mode="HTML")
...
def query_handler(call):
    ...
    if data == "back_home":
        ...
        p = data.split("_")[1]
        toggle_visibility(p)
        query_handler(telebot.types.CallbackQuery(id=call.id, from_user=call.from_user, data="admin_vis_btn", chat_instance=call.chat_instance, message=call.message, json_string=""))
        ...
        server = parts[3]
        gb_val = int(parts[4])
        sent = bot.send_message(cid, f"✍️ <b>Please type the quantity you want to buy (e.g., 5):</b>", parse_mode="HTML")
        ...
        price = int(parts[-1])
        sent = bot.send_message(cid, f"✍️ <b>Please type the quantity you want to buy (e.g., 2):</b>", parse_mode="HTML")
