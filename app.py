...
def check_single_server(server_name, api_url, clean_user_key):
    try:
        keys_response = requests.get(f"{api_url}/access-keys", verify=False, timeout=8)
        ...
        if matched_key:
            ...
            used_bytes = 0
            try:
                metrics_response = requests.get(f"{api_url}/metrics/transfer", verify=False, timeout=8)
