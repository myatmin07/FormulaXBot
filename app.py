...
def get_telegram_photo(user_id):
    try:
        users_db = load_db('formula_x_users.json')
        if str(user_id) in users_db and users_db[str(user_id)].get('photo_url'):
            return users_db[str(user_id)]['photo_url']
        ...
        if data.get('ok') and data['result']['total_count'] > 0:
            ...
            if file_data.get('ok'):
                ...
                if str(user_id) in users_db:
                    users_db[str(user_id)]['photo_url'] = photo_url
                    save_db('formula_x_users.json', users_db)
                return photo_url
    except:
        pass
    return None
...
def check_usage():
    try:
        ...
        match = re.search(r'@([^/:]+):\d+', access_url)
        if not match:
